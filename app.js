...
$("#analyzeLinkBtn").addEventListener("click", () => { const input = $("#videoUrl").value.trim(); const match = input.match(/https?:\/\/[^\s]+/i); if (!match) return status("请粘贴包含 http 或 https 的视频链接。"); const url = match[0].replace(/[，。；;！!]+$/, ""); const isStaticSite = location.hostname.endsWith("vercel.app"); if (isStaticSite) return status("公网版请上传视频文件进行真实扫描。抖音分享链接需要专用后端，暂不在免费静态版中提供。"); loadAndScan(`/proxy-video?url=${encodeURIComponent(url)}`); });
